  ...
  return (
    <>
      ...
      <div className="pointer-events-none fixed bottom-6 left-6 z-10 text-[10px] tracking-[0.35em] uppercase text-white/30">
        R3F · Three.js · GLSL · Bloom · Volumetric Nebula
      </div>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-20 w-[360px] max-w-[92vw]">
          <div className="relative rounded-2xl border border-white/10 bg-black/55 backdrop-blur-xl p-5 shadow-2xl">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-[10px] uppercase tracking-widest text-white/40">
              <div><div className="text-white/85 text-base font-light font-mono">{selected.position[0].toFixed(1)}</div>X</div>
              <div><div className="text-white/85 text-base font-light font-mono">{selected.position[1].toFixed(2)}</div>Y</div>
              <div><div className="text-white/85 text-base font-light font-mono">{selected.position[2].toFixed(1)}</div>Z</div>
            </div>
          </div>
        </div>
      )}
