'use client'
import { useGalaxyStore, type Settings } from '@/lib/store'
...
const FIELDS: Field[] = [
  { key: 'branches',          label: 'Branches',           min: 2,    max: 120,    step: 1,   format: (v) => `${v}` },
  { key: 'spiralTightness',   label: 'Spiral Tightness',   min: 0.2,  max: 3.5,    step: 0.05 },
  { key: 'spin',              label: 'Spin Speed',         min: 0,    max: 3,      step: 0.05 },
  { key: 'count',             label: 'Particle Count',     min: 20000, max: 320000, step: 5000, format: (v) => `${(v/1000).toFixed(0)}k` },
  { key: 'starBrightness',    label: 'Accent Stars',       min: 0,    max: 2,      step: 0.05 },
  { key: 'bloom',             label: 'Bloom Intensity',    min: 0,    max: 2.5,    step: 0.05 },
  { key: 'coreGlow',          label: 'Core Glow',          min: 0,    max: 2,      step: 0.05 },
  { key: 'dustDensity',       label: 'Dust Lanes',         min: 0,    max: 1.5,    step: 0.02 },
  { key: 'nebulaStrength',    label: 'Nebula Haze',        min: 0,    max: 1.5,    step: 0.05 },
  ...
  return (
    <>
      {!open && (
        ...
        >
          <SettingsIcon size={14} className="text-cyan-300" />
          Controls
        </button>
      )}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-cyan-400 shadow-[0_0_10px_#7ee8ff]" />
            <span className="text-[11px] uppercase tracking-[0.35em] text-white/70">Cosmos Tuner</span>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={reset} className="p-1.5 rounded-md text-white/45 hover:text-white hover:bg-white/5" title="Reset to defaults">
              <RotateCcw size={14} />
            </button>
            <button onClick={toggle} className="p-1.5 rounded-md text-white/45 hover:text-white hover:bg-white/5" aria-label="Close controls">
              <X size={14} />
            </button>
          </div>
        </div>
            ...
            return (
              <div key={f.key} className="mb-3.5">
                <div className="flex items-baseline justify-between mb-1">
                  ...
                  <span className="text-[11px] font-mono text-cyan-200/85">{display}</span>
                </div>
                <div className="relative h-1.5 rounded-full bg-white/5 overflow-hidden">
                  <div
                    className="absolute inset-y-0 left-0 bg-gradient-to-r from-cyan-400 via-indigo-500 to-violet-600 transition-all"
                    style={{ width: `${percent}%` }}
                  />
                </div>
                <input
