...
// ─── Soundscapes ───────────────────────────────────────────────────────────────
const SOUNDSCAPES = [
  { id: "void_silence", name: "Void Silence", realm: "shunya",
    desc: "The space between thoughts", bpm: null as number | null, energy: 0.05 },
  { id: "neural_drift", name: "Neural Drift", realm: "medha",
    desc: "Consciousness at rest", bpm: 60, energy: 0.25 },
  { id: "crystal_resonance", name: "Crystal Resonance", realm: "mani_guha",
    desc: "Maṇi Guhā cave harmonics", bpm: null as number | null, energy: 0.15 },
  { id: "vortex_descent", name: "Vortex Descent", realm: "vistara",
    desc: "Falling through the gateway", bpm: 72, energy: 0.45 },
  { id: "cathedral_bloom", name: "Cathedral Bloom", realm: "kathedral",
    desc: "Kathedral Śūnya reverb hall", bpm: null as number | null, energy: 0.20 },
  { id: "solar_ignition", name: "Solar Ignition", realm: "udbhava",
    desc: "Udbhava Śikhara fire rising", bpm: 120, energy: 0.85 },
  { id: "deep_current", name: "Deep Current", realm: "vistara",
    desc: "Pātāla Taraṅga undertow", bpm: 80, energy: 0.55 },
  { id: "pranic_pulse", name: "Prāṇic Pulse", realm: "medha",
    desc: "Ojas vital rhythm tracking", bpm: 72, energy: 0.40 },
]

// ─── Consciousness modes ───────────────────────────────────────────────────────
const CONSCIOUSNESS_MODES = [
  { id: "prajna", name: "Prājña", desc: "Deep focus",
    color: "#2d9e7f", eqCurve: [0,-2,-4,-6,-4,-2,0,2,4,6,4,2] },
  { id: "dhyana", name: "Dhyāna", desc: "Meditation",
    color: "#c4622d", eqCurve: [-6,-4,-2,0,2,4,6,4,2,0,-2,-4] },
  { id: "akshaya", name: "Akṣaya", desc: "Knowledge",
    color: "#00c4cc", eqCurve: [4,6,4,2,0,-2,-4,-2,0,2,4,6] },
  { id: "java", name: "Javā", desc: "High energy",
    color: "#a855f7", eqCurve: [6,4,2,0,-2,-4,-6,-4,-2,0,2,4] },
  { id: "sanchara", name: "Sañcāra", desc: "Transmission",
    color: "#e8b94f", eqCurve: [2,0,-2,-4,-2,0,2,4,6,4,2,0] },
]
...
function useAudioEngine() {
  ...
  const synthesizeSoundscape = useCallback(async (soundscapeId: string) => {
    ...
    if (sc.id === "void_silence") {
      ...
      ;[110, 220, 330, 440, 550, 880].forEach((freq, i) => {
        const osc = ctx.createOscillator()
        osc.type = "sine"; osc.frequency.value = freq + Math.random() * 2
        ...
        lfo.start(); osc.connect(g); g.connect(firstEq); osc.start()
        oscs.push(osc, lfo)
      })
    }
      ...
      left.frequency.value = 200; right.frequency.value = 204
      ...
      left.start(); right.start(); oscs.push(left, right)
      ...
      ;[55, 82.5, 110, 137.5].forEach(freq => {
        ...
        lfo.start(); osc.connect(g); g.connect(firstEq); osc.start(); oscs.push(osc, lfo)
      })
    }
  ...
  const setEQBand = useCallback((bandIdx: number, gainValue: number) => {
    if (eqRef.current[bandIdx]) {
      eqRef.current[bandIdx].gain.setTargetAtTime(gainValue, ctxRef.current?.currentTime || 0, 0.01)
    }
  }, [])
...
function PlasmaRing({ frequencyData, theme, size = 120 }: {
  frequencyData: Uint8Array; theme: RealmTheme; size?: number
}) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bars; i++) {
        ...
        const r1 = baseR - S * 0.04
        const r2 = baseR + amplitude
        const x1 = cx + Math.cos(angle) * r1; const y1 = cy + Math.sin(angle) * r1
        const x2 = cx + Math.cos(angle) * r2; const y2 = cy + Math.sin(angle) * r2
        ...
        ctx.lineWidth = 0.8 + intensity * 1.2; ctx.stroke()
        ctx.restore()
      }
      ...
      for (let i = 0; i < nodeCount; i++) {
        ...
        const nx = cx + Math.cos(angle) * baseR; const ny = cy + Math.sin(angle) * baseR
      ...
      for (let i = 0; i < nodeCount; i++) {
        ...
        const x1 = cx + Math.cos(a1) * baseR; const y1 = cy + Math.sin(a1) * baseR
        const x2 = cx + Math.cos(a2) * baseR; const y2 = cy + Math.sin(a2) * baseR
...
function NeuralEQ({ bands, onBandChange, theme }: {
  bands: number[]; onBandChange: (i: number, v: number) => void; theme: RealmTheme
}) {
  ...
  const handleMouseDown = (i: number, e: React.MouseEvent) => {
    e.preventDefault(); setDragging(i)
  }
  ...
  const points = bands.map((gain, i) => [nodeX(i), nodeY(gain)])
  ...
  return (
    <div style={{ position: "relative" }}>
      <svg ref={svgRef} width={W} height={H}
        ...
        <path d={pathD} fill="none" stroke={theme.primary} strokeWidth={1.5} filter={`drop-shadow(0 0 4px ${theme.glow})`} />
        {bands.map((gain, i) => (
...
function SpatialVisualizer({ frequencyData, theme, panX = 0, panY = 0 }: {
  frequencyData: Uint8Array; theme: RealmTheme; panX?: number; panY?: number
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const rafRef = useRef<number>(0)
  const tRef = useRef(0)
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < 3; i++) {
        const phase = (t * 0.8 + i * 0.33) % 1; const r = phase * 45
...
// ─── BPM pulse indicator ───────────────────────────────────────────────────────
function BPMPulse({ bpm, theme }: { bpm: number | null; theme: RealmTheme }) {
...
// ─── Spectrogram ──────────────────────────────────────────────────────────────
function Spectrogram({ frequencyData, theme }: { frequencyData: Uint8Array; theme: RealmTheme }) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  ...
  useEffect(() => {
    ...
    history.forEach((frame, xi) => {
      frame.forEach((val, yi) => {
        if (yi >= 48) return
        const intensity = val / 255; const alpha = intensity * 0.9
...
export default function AcousticConsole({ realmId = "shunya" }: { realmId?: string }) {
  const theme = REALM_THEMES[realmId] || REALM_THEMES.shunya
  const audio = useAudioEngine()

  const [isOpen, setIsOpen] = useState(false)
  const [activeTab, setActiveTab] = useState("play")
  const [eqBands, setEqBands] = useState(new Array(12).fill(0) as number[])
      ...
      >
        <svg width={22} height={22} viewBox="0 0 22 22" fill="none">
          {[2, 5, 8, 11, 14, 17, 20].map((x, i) => {
            ...
            const animH = audio.isPlaying ? 4 + Math.abs(Math.sin((tick * 0.6 + i) * 1.1)) * 14 : h
            return (
              <rect key={i} x={x} y={(22 - animH) / 2} width={1.5} height={animH}
                rx={0.75} fill={theme.primary} opacity={0.7} />
            )
          })}
        </svg>
      </div>
      ...
      {isOpen && (
        <div style={{
          ...
          <div style={{ padding: "14px 16px", maxHeight: 380, overflowY: "auto" }}>
            ...
            {activeTab === "play" && (
              <div>
                <div style={{ marginBottom: 12 }}>
                  <Spectrogram frequencyData={audio.frequencyData} theme={theme} />
                </div>
                {currentSoundscape && (
                  <div style={{ marginBottom: 10 }}>
                    <BPMPulse bpm={currentSoundscape.bpm} theme={theme} />
                  </div>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  {SOUNDSCAPES.map(sc => {
                    const isActive = audio.currentSoundscape === sc.id
                    const scTheme = REALM_THEMES[sc.realm] || theme
                    return (
                      <button key={sc.id} onClick={() => handleSoundscapeSelect(sc.id)} style={{
                        padding: "10px 10px",
                        background: isActive ? scTheme.primary + "22" : "rgba(255,255,255,0.03)",
                        border: `1px solid ${isActive ? scTheme.primary + "60" : theme.border}`,
                        borderRadius: 8, cursor: "pointer", textAlign: "left",
                        transition: "all 0.2s", boxShadow: isActive ? `0 0 16px ${scTheme.glow}` : "none",
                      }}>
                        <div style={{ fontSize: 9, letterSpacing: "0.2em",
                          color: isActive ? scTheme.primary : "rgba(255,255,255,0.65)",
                          textTransform: "uppercase", fontWeight: 500, marginBottom: 3 }}>{sc.name}</div>
            ...
            {activeTab === "eq" && (
              <div>
                <div style={{ fontSize: 8, letterSpacing: "0.25em", color: theme.primary + "60",
                  ...
                  12-Band Neural Equalizer — drag nodes to sculpt
                </div>
            ...
            {activeTab === "space" && (
              <div>
                <div style={{ fontSize: 8, letterSpacing: "0.25em", color: theme.primary + "60",
                  ...
                  Spatial Audio — position your sound source
                </div>
                <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                  <div style={{ width: 100, height: 100, borderRadius: "50%",
                    border: `1px solid ${theme.border}`, cursor: "crosshair", flexShrink: 0,
                    position: "relative", overflow: "hidden" }}
                    onMouseMove={(e) => {
                      const rect = e.currentTarget.getBoundingClientRect()
                      const x = ((e.clientX - rect.left) / rect.width - 0.5) * 2
                      const y = ((e.clientY - rect.top) / rect.height - 0.5) * 2
                      setPanX(Math.max(-1, Math.min(1, x))); setPanY(Math.max(-1, Math.min(1, y)))
                    }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 8, color: theme.primary + "60", letterSpacing: "0.1em", marginBottom: 8 }}>
                      X: {panX > 0 ? "R" : "L"} {Math.abs(panX * 100).toFixed(0)}%
                    </div>
                    <div style={{ fontSize: 8, color: theme.primary + "60", letterSpacing: "0.1em", marginBottom: 16 }}>
                      Y: {panY > 0 ? "F" : "B"} {Math.abs(panY * 100).toFixed(0)}%
                    </div>
                    <div style={{ fontSize: 8, letterSpacing: "0.2em", color: theme.primary + "60",
                    ...
                    {[
                      ...
                      <button key={b.name} style={{ display: "block", width: "100%", marginBottom: 4,
                        padding: "5px 8px", textAlign: "left", background: "rgba(255,255,255,0.03)",
                        border: `1px solid ${theme.border}`, borderRadius: 6, cursor: "pointer",
                        color: "rgba(255,255,255,0.5)", fontSize: 8, letterSpacing: "0.1em",
                        fontFamily: "system-ui" }}>
                        <span style={{ color: theme.primary }}>{b.name}</span>
                        {" "}{b.freq}Hz — {b.desc}
                      </button>
                    ))}
                  </div>
            ...
            {activeTab === "mode" && (
              <div>
                <div style={{ fontSize: 8, letterSpacing: "0.25em", color: theme.primary + "60",
                  textTransform: "uppercase", marginBottom: 12 }}>
                  Medhā&apos;s Consciousness Modes — reshapes EQ and reverb
                </div>
          ...
          <div style={{ padding: "8px 16px", borderTop: `1px solid ${theme.border}`,
            ...
            {audio.isPlaying && (
              <button onClick={audio.stop} style={{ padding: "4px 10px", background: "transparent",
                border: `1px solid ${theme.border}`, borderRadius: 20, cursor: "pointer",
                color: theme.primary + "60", fontSize: 8, letterSpacing: "0.2em",
                textTransform: "uppercase", fontFamily: "system-ui" }}>Stop</button>
            )}
          </div>
        </div>
      )}
